 ...
 * This module contains the main logic for the role-contact-manager widget
 * 
 */

import muserRpc from "../../lib/rpc.mjs";
import RoleContactManager from "./widget.mjs";
    ...
    for (let zone of zonation_info) {
        ...
        widget.explorer.waitTillPath(path_id).then(async () => {
            ...
            try {
                ...
                const users = await muserRpc.modernuser.role.contact.getUsersInfoFormatted({
                    role: role_item.id,
                    zone: zone.id
                });
        ...
        widget.explorer.stageActions.push({
            ...
            onclick: () => {
                ...
                popup.addEventListener('add', async () => {
                    //Fetch user info
                    let [user] = await muserRpc.modernuser.role.contact.getUsersInfoFormatted({ role: role_item.id, zone: zone.id, specific_user: popup.value.userid })
...
/**
 * This method adds a user with all his actions to the widget
 * @param {object} param0 
            ...
            actions: [
                {
                    ...
                    onclick: () => {
                        let yes_no_popup = new BrandedBinaryPopup({
                            title: `Removing the contact person`,
                            question: `${profile.label} will no longer be a contact person for ${role.label} in ${zone.label}. Is that okay?`,
                            positive: 'Yes',
                            negative: 'No',
                            execute: async () => {
                                await muserRpc.modernuser.role.contact.removeContact(
                                    {
                                        subject: profile.id,
                                        role: role.id,
                                        zone: zone.id
